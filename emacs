...
if [ -d "$CONFIGS" ];
then
    docker run -it --rm $X11CFGS -v $WORKSPACE:/mnt/share/Documents -v $CONFIGS:/mnt/share/configs jinwuzhao/emacs-godev $@
else
    docker run -it --rm $X11CFGS -v $WORKSPACE:/mnt/share/Documents jinwuzhao/emacs-godev $@
fi
